...
private:
    ...
    BFGSMat<Scalar>           m_bfgs;   // Approximation to the Hessian matrix  // here calculte hessian analytically 
    Vector                    m_fx;     // History of the objective function values
    Vector                    m_xp;     // Old x
    Vector                    m_grad;   // New gradient
    Vector                    m_gradp;  // Old gradient
    Vector                    m_drt;    // Moving direction
    Eigen::MatrixXd           m_hess;   // Analytical Hessian 
...
public:
    ...
    inline int minimize(Foo& f, Vector& x, Scalar& fx)
    {
        ...
        fx = f(x, m_grad, m_hess);
    
       // std::cout << m_grad << " initial gradient"<< std::endl;

        Scalar gnorm = m_grad.norm();
       // std::cout << gnorm <<"gradient norm"<< std::endl;
        if(fpast > 0)
            m_fx[0] = fx;
        ...
        //std::cout << step <<" :: initial step size"<< std::endl;
        ...
        for( ; ; )
        {
            ...
            LineSearch<Scalar>::LineSearch(f, fx, x, m_grad, m_hess, step, m_drt, m_xp, m_param);

            // New gradient norm
            
            gnorm = m_grad.norm();

            //std::cout << gnorm << "   norm of global gradient after line search"<<std::endl;
            ...
            // y_{k+1} = g_{k+1} - g_k
             ...
             m_bfgs.apply_Hv(m_grad, -Scalar(1), m_drt, m_hess);
           
             //Eigen::MatrixXd AP = - m_hess.inverse();
